...
namespace CodeFam.Notification.Controllers
{
    ...
    {
        protected IActionResult CreateSuccessResponse<T>(T data, string message, MetaData? meta = null, int statusCode = 200)
        {
            var response = ApiResponse<T>.CreateSuccessResult(data, message, statusCode, meta, HttpContext.Request.Path);
            return StatusCode(statusCode, response);
        }
        protected IActionResult CreateErrorResponse(string message, ErrorCode errorCode, List<ValidationErrorDetail>? details = null)
        {
